...
export class InvoicefyGlobalDashboardComponent {
public barChartOptions: Partial<ChartOptions> = {};// Initially hidden
  ...
  approvedPQTotal: number;
  rejectedPQTotal: number;
  pendingPQTotal: number;
  receivedTAXTotal: number;
  receivedOnlyTaxTotal: number;
  pendingTAXTotal: number;
  overAllTAXTotal: number;
  rejectedPQCount: number;
  approvedPQCount: number;
  pendingPQCount: number;
  pendingTAXCount: number;
  receivedTAXCount: number;
  receivedOnlyTaxCount: number;
  overAllTAXCount: number;
  rejectedONTAXTotal: any;
...
totalAmountAndCount() {
  // Initialize totals and counts
  this.approvedPQTotal = 0;
  this.rejectedPQTotal = 0;
  this.pendingPQTotal = 0;
  this.receivedTAXTotal = 0;
  this.receivedOnlyTaxTotal = 0;
  this.pendingTAXTotal = 0;
  this.overAllTAXTotal = 0;
  this.grandTotalInvoices = 0;

  this.approvedPQCount = 0;
  this.rejectedPQCount = 0;
  this.pendingPQCount = 0;
  this.receivedTAXCount = 0;
  this.receivedOnlyTaxCount = 0;
  this.pendingTAXCount = 0;
  this.overAllTAXCount = 0;
  this.totalInvoiceCount = 0;

  // Calculate totals and counts for PQ invoices
  const pqInvoices = this.allInvoiceList.filter(invoice => invoice.proformaCardHeaderId === "PQ");
  pqInvoices.forEach(invoice => {
    this.grandTotalInvoices += invoice.grandTotal;
    this.totalInvoiceCount++;
    ...
    if (invoice.status === "Approved") {
      this.approvedPQTotal += invoice.grandTotal;
      this.approvedPQCount++;
    } else if (invoice.status === "Rejected") {
      this.rejectedPQTotal += invoice.grandTotal;
      this.rejectedPQCount++;
    } else if (invoice.status === "Pending") {
      this.pendingPQTotal += invoice.grandTotal;
      this.pendingPQCount++;
    }
  });

  // Calculate totals and counts for TAX invoices
  const taxInvoices = this.allInvoiceList.filter(invoice => invoice.proformaCardHeaderId === "TAX");
  taxInvoices.forEach(invoice => {
    this.grandTotalInvoices += invoice.grandTotal;
    this.totalInvoiceCount++;

    if (invoice.status === "Amount Received") {
      this.receivedTAXTotal += invoice.grandTotal;
      this.receivedTAXCount++;
    } else if (invoice.status === "Rejected") {
      this.rejectedONTAXTotal += invoice.grandTotal;
      this.receivedOnlyTaxCount++;
    } else if (invoice.status === "Pending") {
      this.pendingTAXTotal += invoice.grandTotal;
      this.pendingTAXCount++;
    }
  });

  // Calculate Over All TAX
  this.overAllTAXTotal = this.receivedTAXTotal + this.rejectedONTAXTotal + this.pendingTAXTotal;
  this.overAllTAXCount = this.receivedTAXCount + this.receivedOnlyTaxCount + this.pendingTAXCount;

  // Round off the totals
  this.approvedPQTotal = this.roundToOneDecimal(this.approvedPQTotal);
  this.rejectedPQTotal = this.roundToOneDecimal(this.rejectedPQTotal);
  this.pendingPQTotal = this.roundToOneDecimal(this.pendingPQTotal);
  this.receivedTAXTotal = this.roundToOneDecimal(this.receivedTAXTotal);
  this.receivedOnlyTaxTotal = this.roundToOneDecimal(this.receivedOnlyTaxTotal);
  this.pendingTAXTotal = this.roundToOneDecimal(this.pendingTAXTotal);
  this.overAllTAXTotal = this.roundToOneDecimal(this.overAllTAXTotal);
  this.grandTotalInvoices = this.roundToOneDecimal(this.grandTotalInvoices);

  // Log Output
  console.log("Approved PQ Total:", this.approvedPQTotal, "| Count:", this.approvedPQCount);
  console.log("Rejected PQ Total:", this.rejectedPQTotal, "| Count:", this.rejectedPQCount);
  console.log("Pending PQ Total:", this.pendingPQTotal, "| Count:", this.pendingPQCount);
  console.log("Received TAX Total:", this.receivedTAXTotal, "| Count:", this.receivedTAXCount);
  console.log("Rejected TAX Total:", this.receivedOnlyTaxTotal, "| Count:", this.receivedOnlyTaxCount);
  console.log("Pending TAX Total:", this.pendingTAXTotal, "| Count:", this.pendingTAXCount);
  console.log("Over All TAX Total:", this.overAllTAXTotal, "| Count:", this.overAllTAXCount);
}
...
OnClickCard(type) {
  ...
  if (type == '1') {
    ...
    // Approved PQ
    const approvedPQList = this.allInvoiceList.filter(invoice => invoice.status === "Approved" && invoice.proformaCardHeaderId === "PQ");
    this.filteredInvoiceList = approvedPQList;
    setTimeout(() => {
      this.spinner.hide();
    }, 1000);
  } else if (type == '3') {
    // Rejected PQ
    const rejectedPQList = this.allInvoiceList.filter(invoice => invoice.status === "Rejected" && invoice.proformaCardHeaderId === "PQ");
    this.filteredInvoiceList = rejectedPQList;
    setTimeout(() => {
      this.spinner.hide();
    }, 1000);
  } else if (type == '4') {
    // Received TAX
    const receivedTAXList = this.allInvoiceList.filter(invoice => invoice.status === "Amount Received" && invoice.proformaCardHeaderId === "TAX");
    this.filteredInvoiceList = receivedTAXList;
    setTimeout(() => {
      this.spinner.hide();
    }, 1000);
  } else if (type == '5') {
    // Rejected TAX
    const rejectedONTAXList = this.allInvoiceList.filter(invoice => invoice.status === "Rejected" && invoice.proformaCardHeaderId === "TAX");
    this.filteredInvoiceList = rejectedONTAXList;
    setTimeout(() => {
      this.spinner.hide();
    }, 1000);
  } else if (type == '6') {
    // Pending TAX
    const pendingTAXList = this.allInvoiceList.filter(invoice => invoice.status === "Pending" && invoice.proformaCardHeaderId === "TAX");
    this.filteredInvoiceList = pendingTAXList;
    setTimeout(() => {
      this.spinner.hide();
    }, 1000);
  } else if (type == '7') {
    // Over All TAX
    const allTAXList = this.allInvoiceList.filter(invoice => invoice.proformaCardHeaderId === "TAX");
    this.filteredInvoiceList = allTAXList;
    setTimeout(() => {
      this.spinner.hide();
    }, 1000);
  } else if (type === '8') {
    // Pending PQ
    const pendingPQList = this.allInvoiceList.filter(invoice => invoice.status === 'Pending' && invoice.proformaCardHeaderId === 'PQ');
    this.filteredInvoiceList = pendingPQList;
    setTimeout(() => {
      this.spinner.hide();
    }, 1000);
  }
