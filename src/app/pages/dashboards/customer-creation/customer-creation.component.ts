...
export class CustomerCreationComponent {
 @ViewChild('editCustomerTemplate') editCustomerTemplate!: TemplateRef<any>;
  ...
  onStateChange(state: any) {
    ...
    const placeOfSupplyValue = `${state.stateName} - ${state.state_Code}`;
  ...
  openEditModal(customer: any, editCustomerTemplate: TemplateRef<any>): void {
    ...
    const selectedStateObj = this.statesList.find(
      s => s.stateName === customer.customerState
    );
    this.customerEditForm.patchValue({
      ...
      customerState: this.statesList.find(s => s.stateName === selectedCustomer.customerState),
  ...
  newCustomerCreation(newCustomerTemplate: any): void {
    ...
    this.modalService.open(newCustomerTemplate, {
      backdrop: 'static',
      keyboard: false, size: 'lg'
    });
  ...
  savecustomerCreation(model: any) {
    ...
    let creatObj = {
      ...
      "customerState": this.CustomerCreationForm.value.customerState.stateName,
      // "state_Code": this.CustomerCreationForm.value.customerState.stateCode,
      "placeOfSupply": this.CustomerCreationForm.value.placeOfSupply,
  ...
  updateExitCustomer(modal: any): void {
    ...
    let updateObj = {
      ...
      customerState: this.customerEditForm.value.customerState.stateName,
      // state_Code: this.customerEditForm.value.customerState.stateCode,
      placeOfSupply: this.customerEditForm.value.placeOfSupply,
      ...
      if (result.isConfirmed) {
        ...
        this.service.deteleGlobal(deletePayload).subscribe(
          (res: any) => {
            ...
            if (res.status === 200) {  // Changed from 400 to 200 for success
              this.getAllCustomerList();
              Swal.fire({
                title: 'Success',
                text: res.message,
