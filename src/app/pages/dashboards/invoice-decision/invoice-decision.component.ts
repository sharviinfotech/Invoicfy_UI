...
@Component({
  ...
  imports: [CommonModule, FormsModule, ReactiveFormsModule, NgxPrintModule, NgStepperModule, CdkStepperModule,],
  standalone: true,
})
export class InvoiceDecisionComponent {
  ...
  // paginatedInvoices: any;
  pqTaxInvoices: any[] = [];       // for PQ + TAX data
  onlyTaxInvoices: any[] = [];     // for OnlyTAX data
  ...
  ngOnInit(): void {
    this.newInvoiceCreation = this.fb.group({
      ...
      ProformaTypeOfServices: ['', Validators.required],
    ...
    this.getStates();
    this.getAllInvoice();
    this.filteredInvoices = this.pqTaxInvoices || [];
  ...
  openGlobalReviewPopup(invoice: any) {
    ...
    this.modalRef = this.modalService.open(GlobalReviewEditComponent, {
      backdrop: 'static',
  ...
  <div class="invoice-container">
    ...
    <table class="table-bordered">
      ...
      <tr>
        ...
        <td><strong>INVOICE NO:</strong> ${invoiceItem.invoiceUniqueNumber}<br><strong>DATE: </strong>${invoiceItem.header.ProformaInvoiceDate}<br><strong>PAN NO</strong>:${invoiceItem.header.ProformaPanNO}<br><strong>GST NO: </strong>${invoiceItem.header.ProformaGstNumber}<br><strong>Type of Aircraft</strong>:${invoiceItem.header.ProformaTypeOfServices}<br><strong>Seating Capasity</strong>:${invoiceItem.header.ProformaSeatingCapasity}</td>
      </tr>
    </table>
            ...
            <body>
              <div class="border">
                ...
                <table class="table-bordered">
                  ...
                  <tr><td>GST NO : ${invoiceItem.header.ProformaGstNo}</td><td>TYPE OF Aircraft : ${invoiceItem.header.ProformaTypeOfServices}</td></tr>
  ...
  <div class="billing-info">
    ...
    <div style="text-align: center !important;background-color: #6ba3cd !important;">From</div>
      ...
      <p><strong>GST NO:</strong> ${invoiceItem.header.ProformaGstNumber}</p>
       <p><strong>TYPE OF AIRCRAFT:</strong> ${invoiceItem.header.ProformaTypeOfServices}</p>
       <p><strong>SEATING CAPACITY:</strong> ${invoiceItem.header.ProformaTypeOfServices}</p>
    </div>
  </div>
 ...
 <table class="table-bordered">
  ...
  onStepChange(event: StepperSelectionEvent): void {
    if (event.selectedIndex === 0) {
      ...
      // Step 2 → Only TAX
      this.currentPage = 1;
      this.filteredInvoices = this.onlyTaxInvoices;
    }
  ...
  getAllInvoice() {
    ...
    this.service.getAllInvoice().subscribe(
      (res: any) => {
        ...
        this.pqTaxInvoices = this.allInvoiceList.filter(
          (item) =>
            item.proformaCardHeaderId === 'PQ' ||
            item.proformaCardHeaderId === 'TAX'
        );

        this.onlyTaxInvoices = this.allInvoiceList.filter(
          (item) => item.proformaCardHeaderId === 'OnlyTAX'
        );

        // 👇 Set default tab = PQ && TAX
        this.filteredInvoices = this.pqTaxInvoices;

        // Now calculate pagination based on PQ + TAX data
        this.currentPage = 1;
        this.calculateTotalPages();
        this.updatePagination();

        console.log('PQ + TAX invoices:', this.pqTaxInvoices);
        console.log('Only TAX invoices:', this.onlyTaxInvoices);
      },
      (error) => {
        this.spinner.hide();
      }
  ...
  calculateTotalPages() {
    const total = this.filteredInvoices ? this.filteredInvoices.length : 0;
    this.totalPages = Math.ceil(total / this.pageSize);
    this.pages = Array.from({ length: this.totalPages }, (_, i) => i + 1);
  }
  onFileSelected(event: any, invoice: any) {
    ...
    if (!allowedTypes.includes(file.type)) {
      Swal.fire('Invalid File', 'Only PNG, JPG, JPEG, or PDF files are allowed.', 'warning');
      return;
    }
    ...
    reader.onload = async () => {
      ...
      const { isConfirmed } = await Swal.fire({
        title: 'Upload DSC File?',
        text: `Are you sure you want to upload DSC for ${invoice.invoiceUniqueNumber}?`,
        icon: 'question',
        showCancelButton: true,
  ...
  uploadDSCFile(originalUniqueId: string, base64File: string) {
    ...
    this.service.dscfileupload(payload).subscribe({
      next: (res: any) => {
        this.spinner.hide();
        console.log("DSC file upload response:", res);
        ...
        if (res.status) {
          ...
          // ✅ Move to Tab 1 (PQ && TAX)
          setTimeout(() => {
            if (this.stepper) {
              this.stepper.selectedIndex = 0; // 👈 Go to first tab
            }

            // ✅ Refresh the data for PQ && TAX tab
            this.getAllInvoice();
      ...
      error: (err) => {
        ...
        Swal.fire('Error', 'Failed to upload DSC File.', 'error');
      }
    });
  }
  ...
  viewDSCFile(invoice: any) {
    ...
    const base64Data = invoice.DSC_UploadFile.trim();
    ...
    if (fileType === 'application/pdf') {
      ...
      Swal.fire('Unsupported File', 'Cannot preview this file type.', 'error');
    }
  }
