...
@Component({
  selector: 'app-funds-recieved-against',
  templateUrl: './funds-recieved-against.component.html',
  styleUrl: './funds-recieved-against.component.css',
  imports: [CommonModule, FormsModule, ReactiveFormsModule, NgxPrintModule],
  standalone: true,
})
export class FundsRecievedAgainstComponent {
...
resetPagination() {
  ...
  // }
    this.service.getAllInvoice().subscribe((res: any) => {
      ...
      this.allInvoiceList = this.allInvoiceList.map(invoice => {
        return {
          ...invoice,
          balanced: Number(invoice.grandTotal) === Number(invoice.actualAmountReceived)
        };
      });
...
onRowSave(invoice: any): void {
  console.log('Saving invoice data:', invoice);
  console.log('Funds Received Date:', invoice.fundsRecievedDate);
  console.log('Ref UTR:', invoice.refUTR);
  console.log('Actual Amount:', invoice.actualAmountReceived);
  let obj ={
    originalUniqueId:invoice.originalUniqueId,
    fundsRecievedDate:invoice.fundsRecievedDate,
    refUTR:invoice.refUTR,
    actualAmountReceived:invoice.actualAmountReceived
  }
  this.service.updateFromFundsScreen(obj).subscribe((res:any)=>{
    console.log("res",res)

    if(res.status == 200 && res.isValid == true){
